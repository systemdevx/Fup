        ...
        const html = `
            <div class="item-row ${item.expanded ? 'expanded' : ''}" id="row-${item.id}">
                <div class="item-summary" onclick="toggleExpand(${item.id})">
                    ...
                    <div class="col-actions"><button class="btn-remove-icon" onclick="event.stopPropagation(); removerItem(${item.id})"><span class="material-icons-outlined" style="font-size:18px;">close</span></button></div>
                </div>
                <div class="item-details"><div class="form-grid">
                    ...
                    <div class="input-wrapper"><label>DOCUMENTOS</label><div class="file-upload-wrapper" onclick="document.getElementById('file-${item.id}').click()"><span class="file-upload-text" id="filename-${item.id}" style="${arquivoClass}">${arquivoTexto}</span><span class="material-icons-outlined file-icon">description</span></div><input type="file" id="file-${item.id}" style="display:none" onchange="handleFileSelect(${item.id}, this)"></div>
                </div></div>
            </div>`;
